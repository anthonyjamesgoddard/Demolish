...
void crashr::CreateTrunCone(double topRadius, double bottomRadius,
                            double height, int resolution,
                            std::vector<crashr::Vertex>& meshVertices,
                            std::vector<std::array<int, 3>>& meshTriangles) {
    // create coordinates of top
    for (int i = 0; i < resolution; i++)
        meshVertices.push_back(
            Vertex(topRadius * cos(2 * M_PI * (i * 1.0) / resolution), height,
                   topRadius * sin(2 * M_PI * (i * 1.0) / resolution)));
    // create coordinates of bottom
    for (int i = 0; i < resolution; i++)
        meshVertices.push_back(Vertex(
            bottomRadius * cos(2 * M_PI * (i * 1.0) / resolution), -1 * height,
...
void crashr::CreateHopper(double topRadius, double bottomRadius, double height,
                          std::vector<crashr::Vertex>& meshVertices,
                          std::vector<std::array<int, 3>>& meshTriangles) {
    // ***************************
    ...
    // calling CreateTrunCone twice will fill in the meshVertices and
    // meshTriangles. the second time we call this method we should increase the
    // size of the parameters topRadius and bottomRadius so as to give the
    // hopper some thickness.
    int resolution = 4;
    ...
    CreateTrunCone(topRadius + 2, bottomRadius + 2, height, resolution, verts2,
                   tris2);
    ...
    // now we must reverse the triangle ordering in the second call of
    // CreateTrunCone.
