...
crashr::primitives::Cube::Cube(double centre[3], double h)
    : Cube(centre[0], centre[1], centre[2], h) {}

crashr::primitives::Cube::Cube(double centreX, double centreY, double centreZ,
                                 double h)
...
#if DEBUG >= 1
    std::cout << crashr::getOutputPrefix() << "create cube around ("
              << centreX << "," << centreY << ","
              << " with h=" << h << std::endl;
#endif
    discretise();
...
double crashr::primitives::Cube::getCentreX() const { return _centre[0]; }

double crashr::primitives::Cube::getCentreY() const { return _centre[1]; }

double crashr::primitives::Cube::getCentreZ() const { return _centre[2]; }

int crashr::primitives::Cube::getNumberOfTriangles() const { return 2 * 6; }

double crashr::primitives::Cube::getBoundingSphereRadius() const {
    return std::sqrt(3) * _h;
}

void crashr::primitives::Cube::discretise() {
...
const double* crashr::primitives::Cube::getXCoordinates() const {
    assert(_xCoordinates != nullptr);
    assert(_yCoordinates != nullptr);
    assert(_zCoordinates != nullptr);
...
const double* crashr::primitives::Cube::getYCoordinates() const {
    assert(_xCoordinates != nullptr);
    assert(_yCoordinates != nullptr);
    assert(_zCoordinates != nullptr);
...
const double* crashr::primitives::Cube::getZCoordinates() const {
    assert(_xCoordinates != nullptr);
    assert(_yCoordinates != nullptr);
    assert(_zCoordinates != nullptr);
